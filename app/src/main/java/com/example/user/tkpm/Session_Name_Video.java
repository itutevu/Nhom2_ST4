...
public class Session_Name_Video {
    ...
    private static final String PREF_NAME = "Session_Name_Video";

    private static final String KEY_LICHSU = "lichsu";


    public Session_Name_Video(Context context) {
    ...
    public void createSession(List<NameVideoModel> hScripts) {
        if (hScripts == null) {
        ...
        for (int i = 0; i < hScripts.size(); i++) {
            NameVideoModel hScript = hScripts.get(i);
    ...
    public List<NameVideoModel> getListNameVideo() {
        ...
        for (int i = 0; i < NUMBER_H; i++) {
            ...
            if (json.isEmpty())
                continue;
            NameVideoModel hScript = gson.fromJson(json, NameVideoModel.class);
